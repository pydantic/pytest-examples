...
def find_cases():
    ...
    for f in root_dir.iterdir():
        ...
        if f.suffix in {'.py', '.md'}:
            content = f.read_text()
            m = re.search(r'(.+?)^#{5,}', content, flags=re.MULTILINE | re.IGNORECASE | re.DOTALL)
            assert m, f'No EXAMPLE match found for {f}'
            example = m.group(1).strip('\n')
            m = re.search(
                r'^#{5,}\s*output\s*#{5,}\n(.+?)^#{5,}', content, flags=re.MULTILINE | re.IGNORECASE | re.DOTALL
            )
            ...
            m_test_count = re.search(r'^test_count *= *(\d+)', test, flags=re.MULTILINE)
            if m_test_count:
            ...
            if f.suffix == '.md':
                m = re.search(r'^```.*?^(.+?)^```', test, flags=re.MULTILINE | re.DOTALL)
                if m:
                    test = m.group(1)
